...
public class Mediator : IMediator
{
    ...
    public async Task SendAsync<TMessage>(TMessage cmd, CancellationToken cancellationToken = default)
        where TMessage : ICommand
    {
        await SendMessage(cmd, cancellationToken).ConfigureAwait(false);
    }

    public async Task<TResponse> SendAsync<TMessage, TResponse>(TMessage cmd, CancellationToken cancellationToken = default) where TMessage : ICommand where TResponse : IResponse
    {
    ...
    public async Task PublishAsync<TMessage>(TMessage evt, CancellationToken cancellationToken = default)
        where TMessage : IEvent
    {
        await SendMessage(evt, cancellationToken).ConfigureAwait(false);
    }

    public async Task PublishAsync<TMessage>(IReceiveContext<TMessage> receiveContext, CancellationToken cancellationToken = default)
        where TMessage : IEvent
    {
        await SendMessage(receiveContext, cancellationToken).ConfigureAwait(false);
    }
    ...
    public async Task<TResponse> RequestAsync<TRequest, TResponse>(IReceiveContext<TRequest> receiveContext, CancellationToken cancellationToken = default)
        where TRequest : IRequest
        where TResponse : IResponse
    {
